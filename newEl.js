...
let fileSelector = document.getElementById('photo');
    function openFile(event){
        ...
        reader.onload = function(){
            src = reader.result;
            document.getElementsByClassName('label_photo')[0].textContent = 'Фото выбрано';
        };
    ...
    function change_db(){
        ...
        if (check(document.getElementsByName('name')[0].value, document.getElementsByName('description')[0].value,document.getElementsByName('size')[0].value,document.getElementsByName('price')[0].value, name_photo)){
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4) {
                    if (this.status == 200){
                        ...
                        if(answ==true){
                            window.location.href = "index.php?id="+JSON.parse(this.responseText).id;
                        }
                        else{
                            alert(answ);
                        }
            ...
            xmlhttp.open("POST", "new_create.php", true);
